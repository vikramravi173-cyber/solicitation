...
export async function buildFinalReport(
  ...
  const executiveSummary = synthesizeSentences([
    `We profiled all ${catalogProfileCount} entries in the Gov Events & Opportunities catalog against your intake answers, then researched and scored your top ${recommendedOpportunities.length} matches.`,
    top
  ...
  const overallStrategy = synthesizeSentences([
    ...
    `For each pursuit, download the official solicitation, build a compliance matrix, and confirm your stated TRL (${company.technologyReadinessLevel}) meets the expected maturity.`,
